...
from app.commands import basic, instances, network, operations, quota, volumes
...
def build(ctx: AppContext) -> Application:
    ...
    for handler in quota.make_handlers(ctx):
        app.add_handler(handler)
    for handler in network.make_handlers(ctx):
        app.add_handler(handler)
    for handler in volumes.make_handlers(ctx):
        app.add_handler(handler)
